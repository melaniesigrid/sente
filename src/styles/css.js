...
.sente-root {
  /* The palette. Every value here is the house theme, and every one of them is
     overwritten by the shell from src/theme/palettes.js when another room is
...
.foot {
  ...
  padding: 18px clamp(16px, 4vw, 44px); font-size: 13px; letter-spacing: .05em;
  align-items: center;
}
...
.seg-btn {
  ...
  padding: 7px 12px; font: 700 14px var(--font-body); letter-spacing: .04em;
...
.lesson-meta p { font-size: 14px; opacity: .65; margin: 0; }
...
.tier-btn .tier-sub { font-size: 12.5px; opacity: .6; }
...
.fine-inline { font-size: 13px; opacity: .6; }
...
.saying-card .saying-line { font-size: 15.5px; line-height: 1.7; }
...
/* ---- a pull quote: one idea in plain words, set large between the paragraphs ----
   The magazine move. Display italic, wider leading than the prose it interrupts,
   a short accent rule over it, and the label under it so the line is never taken
   for a quotation of the text around it. */
.pull-quote { display: flex; flex-direction: column; align-items: center; gap: 10px; margin: 2px 0; padding: 20px 10px 16px; text-align: center; }
.pull-quote::before { content: ""; width: 38px; height: 2px; border-radius: 2px; background: var(--accent); opacity: .85; }
.pull-line { margin: 0; max-width: 34ch; font-family: var(--font-display-italic); font-style: italic; font-size: clamp(19px, 2.3vw, 24px); line-height: 1.34; letter-spacing: -.012em; }
.pull-label { font-size: 12px; font-weight: 700; letter-spacing: .13em; text-transform: uppercase; opacity: .5; }
@media (max-width: 620px) { .chapter-body { padding-left: 12px; } }

.lobby-creed { max-width: 62ch; margin: 2px auto 0; text-align: center; font-family: var(--font-display-italic); font-style: italic; font-size: 14.5px; line-height: 1.6; opacity: .72; }
...
.closing-saying p { margin: 0; font-size: 15px; line-height: 1.65; }
