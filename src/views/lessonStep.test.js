...
describe("sequence", () => {
  ...
  it("a long sequence waits for the learner instead of a clock", () => {
...
describe("replay", () => {
  it("plays itself to the first stop and opens there, saying nothing yet", () => {
    ...
    expect(s.status).toBe("open");
    // The stop's question is asked in the step text, not said into the log.
    expect(s.log).toEqual([]);
  ...
  it("scores his move in full, keeps the stone, and resumes to the next stop", () => {
    ...
    expect(last(s)).toEqual({ tone: "success", text: "His move." });
    ...
    s = advanceTo(s, 4);
    expect(s.status).toBe("open");
    expect(texts(s)).toEqual(["His move."]);   // the verdict survives the replay moving on
  ...
  it("gives partial credit for a strong move, marks it, and then shows his", () => {
    ...
    expect(last(s)).toEqual({ tone: "verdict", text: "Close.", verdict: "fine" });
    ...
    expect(texts(s)).toEqual(["Close."]);
  });

  it("says why a refutation fails, plays it out, and Try again keeps score and transcript", () => {
    let s = advanceTo(initStep(replayLesson, rstep), 2);
    s = stepReducer(replayLesson, rstep, s, { type: "play", c: 4, r: 4 });
    expect(s.status).toBe("busy");
    expect(last(s)).toEqual({ tone: "correction", text: "Too early." });
    expect(s.pending.action).toEqual({ type: "refute" });
    const said = s.log;
    s = stepReducer(replayLesson, rstep, s, s.pending.action);
    expect(s.status).toBe("review");
    expect(s.log).toEqual(said);
  ...
  it("marks any other point wrong, clears the marker, and keeps the correction", () => {
    let s = advanceTo(initStep(replayLesson, rstep), 2);
    s = stepReducer(replayLesson, rstep, s, { type: "play", c: 0, r: 0 });
    expect(s.status).toBe("wrong");
    expect(last(s)).toEqual({ tone: "correction", text: wrongTextFor(rstep.stops[0]) });
    const said = s.log;
    s = stepReducer(replayLesson, rstep, s, s.pending.action);
    expect(s.status).toBe("open");
    expect(s.log).toEqual(said);
  ...
  it("Show me plays his move at the stop after two misses, and scores nothing for it", () => {
    let s = advanceTo(initStep(replayLesson, rstep), 2);
    expect(canReveal(rstep, s)).toBe(false);
    s = stepReducer(replayLesson, rstep, s, { type: "play", c: 0, r: 0 });
    s = stepReducer(replayLesson, rstep, s, { type: "clearWrong" });
    s = stepReducer(replayLesson, rstep, s, { type: "play", c: 1, r: 1 });
    s = stepReducer(replayLesson, rstep, s, { type: "clearWrong" });
    expect(s.attempts).toBe(REVEAL_AFTER);
    expect(canReveal(rstep, s)).toBe(true);
    s = stepReducer(replayLesson, rstep, s, { type: "reveal" });
    expect(s.status).toBe("scored");
    expect(s.revealed).toBe(true);
    expect(s.score).toBe(0);
    expect(s.stopsDone).toBe(1);
    ...
    s = advanceTo(s, 4);
    expect(s.status).toBe("open");
    // A new stop starts the miss count over: the study cannot be revealed wholesale.
    expect(canReveal(rstep, s)).toBe(false);
  });
  ...
  it("finishes solved at the end of the moves with the step's success text", () => {
    ...
    expect(texts(s)).toEqual(["His move.", "Yes.", "The end of the excerpt."]);
    expect(s.pending).toBeNull();
  });
...
describe("maxim", () => {
  ...
  it("starts solved and locked, with its marks and nothing said", () => {
  ...
  it("is read, not solved, so it never offers a reveal", () => {
    expect(canReveal(step, { ...initStep(replayLesson, step), attempts: 9 })).toBe(false);
  });
});

describe("every step in the library can be finished without guessing", () => {
  it("reveal solves every non-info step that stands on its own", () => {
    for (const lesson of LIBRARY) {
      for (const step of lesson.steps) {
        if (step.type === "info" || step.type === "maxim" || step.type === "replay") continue;
