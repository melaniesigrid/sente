...
export function Game({ mode, onExit, profile, setProfile, notify, initial }) {
  ...
  /* Deja vu: the board saying you have been here before. Off during scoring and
     once the game is over, where the reader is looking at a result rather than
     at a position, and off entirely for anybody who has turned it off. */
  /* Memoised on the position rather than computed per render: canonical() turns
     the board over eight times and hashes each one, and a running clock renders
     this view several times a second between moves. */
  const deja = useMemo(
