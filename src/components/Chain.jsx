...
export function ChainStrip({ profile, today = dayKey(), days = 28 }) {
  ...
  return (
    <div className="chain-strip" role="img"
         aria-label={t("chain.strip", { count: kept, days },
           `${kept} ${kept === 1 ? "day" : "days"} practised in the last ${days}`)}>
      {marks.map((m, i) => (
        <span key={m.key}
  ...
  return (
    <div className="chain-line">
      <div className="chain-run">
        <Flame size={16} />
        <span className="stat-num">{run.days}<em>{t("chain.days", { count: run.days }, run.days === 1 ? "day" : "days")}</em></span>
      </div>
      <div className="chain-side">
        <ChainStrip profile={profile} today={today} />
        <span className="fine">{chainNote(run, t)}</span>
      </div>
    </div>
  );
}
...
export function ChainYear({ profile, today = dayKey(), weeks = 26 }) {
  ...
  return (
    <div className="chain-year" role="img"
         aria-label={t("chain.year", { count: kept, weeks },
           `${kept} ${kept === 1 ? "day" : "days"} practised in the last ${weeks} weeks`)}>
      {marks.map(mk => (
        <span key={mk.key}
