...
assert(st.room.seats.b.name === "Ada L" && st.room.record.phase === "playing", "room state on join");
const seatA = await ga.next(m => m.t === "seat");
assert(seatA.seat === "b", "ada seated as black");
...
assert(ch.msg.text === "hello" && ch.msg.seat === "w", "chat relays");
...
assert(u.room.undo && u.room.undo.by === "w", "black sees the undo request");
